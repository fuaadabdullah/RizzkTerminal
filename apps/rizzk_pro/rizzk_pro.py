...
def upsert_watchlist(symbol: str, note: str) -> None:
    ...
    conn.execute(
        """
        INSERT INTO watchlist (symbol, note, created_at, updated_at)
...
user_openai = st.sidebar.text_input(
    "OpenAI API Key (sk-...)", value=default_openai, type="password"
)
user_alpha = st.sidebar.text_input(
    "Alpha Vantage API Key", value=default_alpha, type="password"
)
if user_openai:
...
st.sidebar.caption(
    "Tip: persist with `setx OPENAI_API_KEY ...` then reopen PowerShell."
)

risk_per_trade = st.sidebar.slider(
    "Risk per trade (journal context)", 0.0, 0.05, 0.01, 0.001
)
...
with tabs[0]:
    ...
    with c2:
        period = st.selectbox(
            "Period", ["1mo", "3mo", "6mo", "1y", "5y", "max"], index=2
        )
    with c3:
    ...
    if st.button("Fetch", type="primary", key="fetch_px"):
        ...
        if df.empty:
            ...
            if show_rsi:
                rs = rsi(df["Close"], 14)
                c = (
                    alt.Chart(pd.DataFrame({"Date": df["Date"], "RSI": rs}))
                    .mark_line()
                    .encode(
    ...
    if watch_rows:
        ...
        st.caption("No saved watchlist entries yet.")

    with st.form("watchlist_form", clear_on_submit=True):
        w_symbol = st.text_input("Symbol to track", value="", key="watchlist_symbol")
        w_note = st.text_input("Note", value="", key="watchlist_note")
        submitted = st.form_submit_button("Save symbol")
        if submitted:
            if not w_symbol.strip():
                st.warning("Enter a symbol before saving to the watchlist.")
            else:
                upsert_watchlist(w_symbol, w_note)
                st.success(f"Saved {normalize_symbol(w_symbol)} to the watchlist.")
    ...
    if st.button("Run Backtest", type="primary"):
        ...
        if df.empty:
            ...
            plot_df = pd.DataFrame(
                {"Date": df["Date"], "Buy&Hold": eq_bh, "Strategy": eq_st}
            )
            line = (
                alt.Chart(plot_df.melt("Date"))
                .mark_line()
...
with tabs[2]:
    ...
    opt = st.selectbox(
        "List", ["Trending", "Most Active", "Top Gainers", "Top Losers"], index=0
    )
...
with tabs[3]:
    ...
    with colA:
        j_text = st.text_area(
            "New note", height=120, placeholder="Plan / review / psychology."
        )
    with colB:
        j_symbol = (
            st.text_input("Symbol (optional)", value="", key="j_symbol").upper().strip()
        )
        j_tag = st.selectbox(
            "Tag", ["Plan", "Review", "Psychology", "Stats", "Other"], index=0
        )
    ...
    if st.button("Add Note", type="primary"):
        ...
        JOURNAL_PATH.write_text(
            json.dumps(st.session_state.journal, indent=2), encoding="utf-8"
        )
...
with tabs[4]:
    ...
    if st.button("Fetch News", key="news_fetch"):
        feed = fetch_av_news(
            tuple(s.strip() for s in tickers.split(",") if s.strip()), limit=30
        )
        if not feed:
...
with tabs[5]:
    ...
    if not openai_available:
        st.warning(
            "OpenAI SDK not installed. Install `openai` to enable the assistant."
        )
    else:
        if "messages" not in st.session_state:
            st.session_state.messages = [
                {
                    "role": "system",
                    "content": "You are a concise, realistic trading assistant. Be practical. Explain entries/exits, risk, and how to use this app.",
                }
            ]
